...
{
    public partial class frm6PhieuMuon : Form
    {
        public frm6PhieuMuon()
        {
            InitializeComponent();
        }
        SqlConnection cnn;
        private void ketnoi()
        {
            string kenoi = @"Data Source=HD;Initial Catalog=DA_QLTV;Integrated Security=True";
        ...
        private void insert_QL()
        {
            ...
            string s2 = pn2hantra.Value.Year + "/" + pn2hantra.Value.Month + "/" + pn2hantra.Value.Day;
            SqlCommand cmd = new SqlCommand("insert into QL_PHIEUMUON (MaPM,MaSach,HanTra,SoLuong) values('" + pn2mapm.Text + "','" + pn2masach.Text + "',convert(smalldatetime,'" + s2.ToString() + "'),'" + tbsl.Text + "')", cnn);
            cmd.ExecuteNonQuery();
        }
        ...
        private void bttthemmoi_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception) { MessageBox.Show("Vui lòng nhập đủ thông tin."); }
        }
       ...
       private void button22_Click(object sender, EventArgs e)
        ...
        private void search()
        {
            ...
            cmd.CommandText = "select* from PHIEUMUON where MaPM like '%" + tbtk.Text + "%' or MaDG like N'%" + tbtk.Text + "%' or NgayMuon like '%" + tbtk.Text + "%' or MaNV like '%" + tbtk.Text + "%'";
        ...
        {
            DialogResult f = MessageBox.Show("Bạn có chắc muốn xóa không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (f == DialogResult.Yes)
            {
                try
                {
        ...
        private void dgvpn2_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            int i = e.RowIndex;
            try
            {
                ...
                tbsl.Text = dgvpn2.Rows[i].Cells[3].Value.ToString().Trim();
            }
            catch (Exception) { }
        }
