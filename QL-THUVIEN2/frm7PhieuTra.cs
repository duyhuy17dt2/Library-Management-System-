...
{
    public partial class frm7PhieuTra : Form
    {
        public frm7PhieuTra()
        {
            InitializeComponent();
        ...
        private void ketnoi()
        {
            cnn = new SqlConnection(@"Data Source=HD;Initial Catalog=DA_QLTV;Integrated Security=True");
            cnn.Open();
        }
        ...
        private void update_TS()
        {
            ...
            int songayquahan = (y.Day - pnngaytra.Value.Day);
            int sotienphat = songayquahan * 5000;

            string s = pnngaytra.Value.Year + "/" + pnngaytra.Value.Month + "/" + pnngaytra.Value.Day;
            SqlCommand cmd = new SqlCommand("update TRASACH set NgayTra=convert(smalldatetime,'" + s.ToString() + "'),PhatQuaHan='" + sotienphat + "' where MaPM='" + pnmaphieu.Text + "' and MaSach='" + pnmasach.Text + "' ", cnn);
            cmd.ExecuteNonQuery();
        ...
        private void bttk_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "select* from TRASACH where MaPM like '%" + tbtk.Text + "%' or MaSach like N'%" + tbtk.Text + "%' or MaNV like '%" + tbtk.Text + "%' or NgayTra like '%" + tbtk.Text + "%'";
        ...
        private void lbht_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            HienThi();
        }
